...
def save_upload_file_tmp(upload_file: UploadFile, config: Settings) -> Path:
    try:
        ...
        with NamedTemporaryFile(delete=False, suffix=suffix, dir=config.media_base) as tmp:
...
async def convert_and_upload(
    ...
    src_image = (
        str(save_upload_file_tmp(image, config))
        if image
        else config.default_a2v_image
    )
    chained_task = chain(task_convert_to_audio.s(src_audio_path, src_image), task_upload_to_youtube.s(email, **snippet.dict()))
...
async def upload_to_youtube(
    video: UploadFile, email: str = Form(...), snippet: Snippet = Depends(), config: Settings = Depends(get_config),
):
    src_video = str(save_upload_file_tmp(video, config))
    task_upload_to_youtube.apply_async(
